...
<html lang = "en">
	...
	<body>
		...
		<div class="topnav">
			...
			<a class="active" href="PPPCancelOrder.php">Cancel A Client’s Order</a>
        ...
        <?php
            ...
            // POST
            if ($_SERVER["REQUEST_METHOD"] == "POST"  && isset($_POST['cid'],$_POST['cOrderNum'])) {
                $clientid=$_POST["cid"];
				$cOrderNum=$_POST["cOrderNum"];

				// Make form global for later use
				if(isset($_POST['cid'],$_POST['cOrderNum'])){
					$_SESSION["cid3"]=$clientid;
					$_SESSION["cOrderNum"]=$cOrderNum;
				}

                // Check if client exists
                $queryString = "SELECT * FROM PPPOrders WHERE orderNumber='$cOrderNum' AND clientID='$clientid'";
				...
				if($result->num_rows){		
                    ...
                    echo '<script>alert("Either Order Number or Client ID do not exist.")</script>';
                }
            }
			// POST hidden value
			if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['inputBoolean'])){
				...
				$clientid=$_SESSION["cid3"];
				$cOrderNum=$_SESSION["cOrderNum"];

				if(strcmp($confirmVar, 'true') == 0){
					$delete = "DELETE FROM PPPOrders WHERE orderNumber='$cOrderNum' AND clientID='$clientid'";
